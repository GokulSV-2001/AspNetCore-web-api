...
namespace NZwalks.Api.Repositories
{
    public class SQLWalkRepository : IWalkRepository
    {
        ...
        public async  Task<List<Walk>> GetAllWalksAsync(string? filterOn = null, string? filterQuery = null,string? sortBy=null,bool ascending=true,
        int pageNumber=1,int pageSize=1000)
        {
            ...
            if (String.IsNullOrWhiteSpace(sortBy) == false)
            {
                ...
                {
                    ("NAME")=>ascending?walks.OrderBy(x=>x.Name):walks.OrderByDescending(x=>x.Name),
                    ("DESCRIPTION")=>ascending?walks.OrderBy(x=>x.Description):walks.OrderByDescending(x=>x.Description),
                    ("LENGTHINKM")=>ascending?walks.OrderBy(x=>x.LengthInKm):walks.OrderByDescending(x=>x.LengthInKm),
                    _=>walks
                };
            }
            ...
            var skipresult=(pageNumber-1)*pageSize;
             ...
             return  walks.Skip(skipresult).Take(pageSize).ToList();
        }
